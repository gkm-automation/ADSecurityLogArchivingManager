...
$global:databasename = "OfflineEventDB"
...
$queryserver = Invoke-Sqlcmd2 -ServerInstance $global:dbinstance -Database $global:databasename -Query " SELECT DISTINCT machinename FROM NodeInfo ORDER BY Machinename "
        if(!($queryserver)) { 
        ...
        $Results = ($Log.BackupEventlog($Archivepath)).ReturnValue
            If($Results -eq 0) {
                ...
                $logclear = ($Log.ClearEventlog()).ReturnValue
                    ...
                    $NodeInfotbcheck = Invoke-Sqlcmd2 -ServerInstance $dbinstance -Database $databasename -Query " SELECT  top 1 machinename FROM NodeInfo where machinename LIKE '%$Servername%'"
                    ...
                    if(!($NodeInfotbcheck)){
                        ...
                        $updatequery =  @"
                        INSERT INTO [dbo].[NodeInfo](Machinename,Lastupdated,RecentFilename)VALUES ('$servername_db','$convertsqltime','$ArchiveFile')
"@ 
                    }
                    else{
                    Write-Log -Message "Last backup details refreshed in Node info Table"
                    $updatequery =  @"
                    update  [dbo].[NodeInfo] set LastUpdated='$convertsqltime',RecentFilename='$ArchiveFile' where machinename LIKE '%$Servername%'
"@ 
                    }
